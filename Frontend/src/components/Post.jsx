...
function Post({ post,setPosts, active ,user}) {
  
  async function deleteHandler() {
    setPosts((prevPosts) => prevPosts.filter((p) => p.post_id !== post.post_id));
    await axios.delete(`http://localhost:9000/post/deletePost/${post.post_id}`, {
      ...
      <div className="flex w-full justify-between items-center">
        ...
        {active && (
          <div className="flex gap-5">
            ...
            <MdDelete className="text-2xl text-red-500 cursor-pointer" onClick={deleteHandler} />
          </div>
        )}
      </div>
