...
const hashedPassword =  await hashPassword(person_password);
  try{
    const [p]=await db.execute(
      ...
      [person_id,person_username,person_fname,person_lname,person_dob,person_gender,hashedPassword]
    );
    res.status(201).json(new ApiResponse(true, "User registered successfully", null));
   }catch(error){
